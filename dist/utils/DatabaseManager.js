...
export default class DatabaseManager {
    ...
    static async logStockPurchase(ticker, date, price, quantity) {
        await this.connect();
        try {
    ...
    static async reduceStockPurchase(ticker, date, quantity) {
        await this.connect();
        try {
            ...
            if (newQuantity > 0) {
                await this.stockPurchases.updateOne({ ticker }, { $set: { quantity: newQuantity } });
